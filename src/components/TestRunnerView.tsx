...
export const TestRunnerView: React.FC = () => {
  ...
  return (
    <div className="bg-white rounded-xl border border-slate-200/80 overflow-hidden shadow-2xs">
      {/* Header */}
      <div className="p-4 border-b border-slate-100 bg-slate-50/60 flex flex-wrap items-center justify-between gap-4">
        <div>
        ...
        <div className="flex items-center gap-2">
          <button
            ...
            disabled={isPending}
            className="inline-flex items-center gap-2 px-3 py-1.5 bg-slate-900 hover:bg-slate-800 text-white rounded-lg text-xs font-medium transition-colors disabled:opacity-50 cursor-pointer"
          >
            {isPending ? (
      ...
      {report && (
        <div className="grid grid-cols-2 sm:grid-cols-4 gap-3 p-4 border-b border-slate-100 bg-white">
          <div className="p-3 bg-slate-50/70 rounded-lg border border-slate-200/80">
            <div className="text-[11px] font-medium text-slate-400 uppercase tracking-wide">Total Tests</div>
            <div className="text-lg font-semibold text-slate-900 mt-0.5">{report.total}</div>
          </div>
          <div className="p-3 bg-emerald-50/50 rounded-lg border border-emerald-200/60">
            ...
            <div className="text-lg font-semibold text-emerald-700 mt-0.5 flex items-center gap-1.5">
          ...
          <div className="p-3 bg-rose-50/50 rounded-lg border border-rose-200/60">
            ...
            <div className="text-lg font-semibold text-rose-700 mt-0.5 flex items-center gap-1.5">
              <span>{report.failed}</span>
              {report.failed > 0 && <XCircle className="w-4 h-4 text-rose-600" />}
            </div>
          </div>
          <div className="p-3 bg-slate-50/70 rounded-lg border border-slate-200/80">
            <div className="text-[11px] font-medium text-slate-400 uppercase tracking-wide">Duration</div>
            <div className="text-lg font-semibold text-slate-900 mt-0.5">{report.durationMs} ms</div>
          </div>
        </div>
      )}

      {/* Filters */}
      <div className="px-4 py-2.5 border-b border-slate-100 bg-slate-50/40 flex items-center justify-between text-xs">
